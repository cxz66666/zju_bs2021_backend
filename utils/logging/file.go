...
// getLogFilePath returns the folder which contains the logs file, according to the project root path
// default is /runtime/logs
func getLogFilePath() string {
	return fmt.Sprintf("%s%s",setting.AppSetting.RuntimeRootPath,setting.AppSetting.LogSavePath)
}

//getLogFileName returns a file name, you can change the default style in app.ini, default is log20010101.log
func getLogFileName() string {
	return fmt.Sprintf("%s%s.%s",setting.AppSetting.LogSaveName,time.Now().Format(setting.AppSetting.TimeFormat),
		setting.AppSetting.LogFileExt)
}

// openLogFile help you create the folder according the filePath (if not exists), create and open the logFile according to the fileName,
